 ...
 * 
 * Robotium has full support for WebViews, Activities, Dialogs, Toasts, Menus
 * and Context Menus.
 * 
 * When writing tests there is no need to plan for or expect new activities in
 * the test case. All is handled automatically by Robotium-Solo. Robotium-Solo
 * can be used in conjunction with ActivityInstrumentationTestCase2. The test
 * cases are written from a user perspective were technical details are not
 * needed.
 * 
 * 
...
public class Solo {
	 ...
	 * @param activity
	 *            the start {@link Activity} or {@code null} if no start
	...
	public Solo(Instrumentation instrumentation, Activity activity) {
		...
		this.activityUtils = new ActivityUtils(instrumentation, activity,
				sleeper);
		...
		this.webUtils = new WebUtils(instrumentation, activityUtils,
				viewFetcher, sleeper);
		this.scroller = new Scroller(instrumentation, activityUtils,
				viewFetcher, sleeper);
		this.searcher = new Searcher(viewFetcher, webUtils, scroller, sleeper);
		this.waiter = new Waiter(activityUtils, viewFetcher, searcher,
				scroller, sleeper);
		...
		this.clicker = new Clicker(activityUtils, viewFetcher, sender,
				instrumentation, sleeper, waiter, webUtils);
		this.presser = new Presser(clicker, instrumentation, sleeper, waiter);
		this.textEnterer = new TextEnterer(instrumentation, activityUtils,
				clicker);
	}
	...
	/**
	 * Waits for a View of a certain class to be shown. Default timeout is 20
	 * seconds.
	 * 
	 * @param viewClass
	 ...
	 * @return {@code true} if the {@link WebElement} is shown and {@code false}
	 *         if it is not shown before the timeout
	 * 
	 */
	...
	public boolean searchButton(String text) {
		return searcher
				.searchWithTimeoutFor(Button.class, text, 0, true, false);
	}
	...
	public void clickOnWebElement(WebElement webElement) {
		if (webElement == null)
			Assert.assertTrue(
					"WebElement is null and can therefore not be clicked!",
					false);
	 ...
	 *            the {@link View} to be clicked
	 * @param immediately
	 *            true if view is to be clicked without any wait
	 */
	 ...
	 * 
	 * @param view
	 *            the {@link View} to be long clicked
	 * 
	 */
	 ...
	 * objects that the list line is showing. Will use the first ListView it
	 * finds.
	 * 
	 * @param line
	...
	/**
	 * Clicks on a given list line on a specified ListView and returns an
	...
	/**
	 * Long clicks on a given list line on a specified ListView and returns an
	 * ArrayList of the TextView objects that the list line is showing.
	 * 
	 * @param line
	 *            the line to be clicked
	 ...
	 * 
	 * @param list
	 *            the {@link AbsListView} to be scrolled
	...
	public View getView(int id, int index) {
		...
		if (viewToReturn == null) {
			int match = index + 1;
			if (match > 1) {
				Assert.assertTrue(match + " Views with id: '" + id
						+ "' are not found!", false);
			} else {
				Assert.assertTrue("View with id: '" + id + "' is not found!",
						false);
			}
		}
		return viewToReturn;
			...
			if (match > 1) {
				Assert.assertTrue(
						match
								...
								+ webUtils.splitNameByUpperCase(by.getClass()
										.getSimpleName()) + ": '"
								+ by.getValue() + "' are not found!", false);
			} else {
				Assert.assertTrue(
						"WebElement with "
								+ webUtils.splitNameByUpperCase(by.getClass()
										.getSimpleName()) + ": '"
								+ by.getValue() + "' is not found!", false);
			}
		}
		return webElement;
	}
	...
	/**
	 * Returns an ArrayList of the Views currently shown in the focused Activity
	 * or Dialog.
	 * 
	...
	/**
	 * Returns an ArrayList of Views of the specified Class located in the
	...
	/**
	 * Returns an ArrayList of Views of the specified Class located under the
	 * specified parent.
	 * 
	 * @param classToFilterBy
	 *            return all instances of this class. Examples are
	...
	/**
	 * Returns an ArrayList of WebElements currently shown in the active
	 * WebView.
	 * 
	 * @return an {@code ArrayList} of the {@link WebElement} objects currently
	 *         shown in the active WebView
	 */
	...
	/**
	 * Returns an ArrayList of WebElements of the specified By object currently
	 * shown in the active WebView.
	 * 
	 * @param by
	 *            the By object. Examples are By.id("id") and By.name("name")
	 ...
	 * 
	 * @param text
	 *            the text that the {@link RadioButton} shows, specified as a
	 *            regular expression
	 ...
	 * 
	 * @param text
	 *            the text that the {@link ToggleButton} shows, specified as a
	 *            regular expression
	 * @return {@code true} if a {@link ToggleButton} with the given text is
	 *         checked and {@code false} if it is not checked
	 * 
	 ...
	 * 
	 * @param text
	 *            the text that the {@link CheckBox} shows, specified as a
	 *            regular expression
	 ...
	 *            {@link CompoundButton} objects show, specified as a regular
	 *            expression
	...
	public boolean isTextChecked(String text) {
		...
		if (viewFetcher.getCurrentViews(CheckedTextView.class).size() > 0
				&& checker.isCheckedTextChecked(text))
			return true;

		if (viewFetcher.getCurrentViews(CompoundButton.class).size() > 0
	 ...
	 * 
	 * @param activityClass
	 *            the class of the {@code Activity} to wait for e.g.
	 *            {@code "MyActivity"}
	 * @param timeout
	 *            the amount of time in milliseconds to wait
	 * @return {@code true} if {@link Activity} appears before the timeout and
	...
	/**
	 * Waits for a Fragment with a given tag to appear. Default timeout is 20
	 * seconds.
	 * 
	 * @param tag
	 *            the name of the tag
	...
	/**
	 * Waits for a Fragment with a given id to appear. Default timeout is 20
	 * seconds.
	 * 
	 * @param id
	 ...
	 * Requires read logs permission (android.permission.READ_LOGS) in
	...
	/**
	 * Takes a screenshot and saves it with a given name in
