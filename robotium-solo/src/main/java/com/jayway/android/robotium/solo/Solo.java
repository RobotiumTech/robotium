...
/**
 * Contains all the methods that the sub-classes have. It supports test cases
 * that span over multiple activities.
 * 
 * Robotium has full support for Activities, Dialogs, Toasts, Menus and Context
 * Menus.
 * 
...
public class Solo {
	...
	public Solo(Instrumentation instrumentation, Activity activity) {
		...
		this.clicker = new Clicker(viewFetcher, scroller, robotiumUtils,
				instrumentation, sleeper, waiter);
	...
	public boolean waitForText(String text, int minimumNumberOfMatches,
		...
		return waiter
				.waitForText(text, minimumNumberOfMatches, timeout, scroll);
	}
	...
	public boolean scrollDownList(int index) {
		return scroller.scrollList(ListView.class,
	 ...
	 * 
	 * @param parent
	 *            the parent {@link View} from which the {@link ImageView}
