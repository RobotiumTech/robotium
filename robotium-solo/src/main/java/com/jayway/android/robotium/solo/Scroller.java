...
class Scroller {
	...
	public void scrollToSide(Side side, float scrollLength) {
		...
		int screenWidth = activityUtils.getCurrentActivity(false).getWindowManager().getDefaultDisplay()
				.getWidth();
		float x = screenWidth * scrollLength;
	...
	public void scrollViewToSide(View view, Side side, float scrollLength) {
		int[] corners = new int[2];
		view.getLocationOnScreen(corners);
		int viewHeight = view.getHeight();
		int viewWidth = view.getWidth();
		float x = corners[0] + viewWidth * scrollLength;
